...
import { Phone, MessageCircle, Mail, CreditCard, Leaf, Sparkles } from 'lucide-react';
import { DETAILED_FREE_SERVICES, DETAILED_FREE_SYSTEMS, DETAILED_PAID_SERVICES } from '../constants';
    ...
    <div className="bg-[#f0f4f0] min-h-screen pb-20 font-sans">
      ...
      <div className="bg-white/50 border-b border-gray-100">
        <div className="container mx-auto px-4 py-3 text-[10px] text-gray-400">
          <Link to="/" className="hover:text-salon-pink">トップ</Link> &gt; <span className="text-gray-600 font-bold">サービス詳細・料金</span>
        </div>
      </div>
        ...
        <section className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {DETAILED_PAID_SERVICES.map((service, idx) => (
              <ServiceBox key={idx} item={service} themeColor="#f69494" isPaid />
            ))}
          </div>
        </section>
        ...
        <section className="mb-20 py-20 relative text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto mb-10">
            ...
            <div className="bg-white border-2 border-red-400 p-6 rounded-lg shadow-md flex flex-col justify-center min-h-[160px]">
              <p className="text-red-400 font-bold text-xl mb-1">初期費用5万円</p>
              <p className="text-[10px] text-red-300">通常10万円のところ<br />期間限定特別価格実施中</p>
            </div>
            {/* Box 2 */}
            <div className="bg-white border-2 border-salon-gold p-6 rounded-lg shadow-md flex flex-col justify-center min-h-[160px]">
              <p className="text-gray-700 font-bold text-lg mb-1">月額基本料金3万円</p>
              <p className="text-[10px] text-gray-400">上記無料サービス・システムを<br />全て含みます<br />ルーム数等による変動なし</p>
            </div>
            {/* Box 3 */}
            <div className="bg-white border-2 border-salon-blue p-6 rounded-lg shadow-md flex flex-col justify-center min-h-[160px]">
              <p className="text-salon-blue font-bold text-xl mb-1">1成約：1,000円</p>
              <p className="text-[10px] text-salon-blue/60">問合せ・キャンセルは<br />含みません</p>
            </div>
          </div>

          <div className="text-red-400 text-xs font-bold space-y-1 mb-16">
            <p>※別途消費税10%</p>
            <p>※月間予約数500件以上で成約単価100円、1000件以上で単価200円の割引をさせていただきます。</p>
          </div>

          {/* Bottom CTA */}
          <div className="bg-white/80 backdrop-blur-sm p-10 rounded-[40px] shadow-xl border border-salon-tan max-w-2xl mx-auto">
            <p className="text-red-400 font-bold text-lg mb-8 tracking-widest">ご相談・お見積りはこちら</p>
            <div className="flex flex-col gap-3">
              <a href="tel:050-5443-1770" className="bg-[#f69494] text-white py-4 rounded-xl flex flex-col items-center justify-center hover:opacity-90 transition-opacity">
                 <div className="flex items-center gap-2 font-bold"><Phone size={20} /> 050-5443-1770</div>
                 <span className="text-[10px] opacity-90 mt-1">受付時間 9:30 - 翌4:00</span>
              </a>
              <div className="grid grid-cols-2 gap-3">
                <a href="#" className="bg-white border-2 border-[#f69494] text-[#f69494] py-3 rounded-xl flex flex-col items-center justify-center hover:bg-red-50 transition-colors">
                  <div className="bg-[#f69494] text-white px-2 rounded-full text-[10px] font-bold h-5 flex items-center mb-1">LINE</div>
                  <span className="text-[10px] font-bold">24時間受付</span>
                </a>
                <Link to="/contact" className="bg-[#f69494] text-white py-3 rounded-xl flex flex-col items-center justify-center hover:opacity-90 transition-opacity">
                  <span className="text-[11px] font-bold leading-tight">お問い合わせフォーム</span>
                  <span className="text-[10px] opacity-90 mt-1">24時間受付</span>
                </Link>
              </div>
            </div>
          </div>
      ...
      <div className="container mx-auto px-4 max-w-5xl mt-12 pb-12">
        <nav className="text-[10px] text-gray-400">
          <Link to="/" className="hover:text-salon-pink">トップ</Link> &gt; <span className="text-gray-500">サービス詳細・料金</span>
        </nav>
      </div>
    </div>
  );
...
const ServiceBox: React.FC<{ item: any, themeColor: string, isPaid?: boolean }> = ({ item, themeColor, isPaid }) => {
  return (
    <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm flex flex-col items-center text-center group hover:shadow-md transition-shadow h-full">
      <div className="mb-4 relative">
        ...
        <div className="flex flex-col items-center">
           <div className={`p-2 rounded-full mb-1 ${isPaid ? 'text-[#f69494]' : 'text-salon-gold'}`}>
             {item.icon}
           </div>
           <h3 className={`font-bold text-sm tracking-tight ${isPaid ? 'text-[#f69494]' : 'text-salon-gold'}`}>
             {item.title}
           </h3>
        </div>
      </div>
