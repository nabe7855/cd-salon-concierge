...
import { Search, ChevronRight, Clock, User, Facebook, Twitter, MessageCircle, Copy, Bookmark, Share2, CornerDownRight } from 'lucide-react';
      ...
      <div className="container mx-auto px-4 py-8 lg:py-12 flex flex-col lg:flex-row gap-12 max-w-7xl">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 text-[10px] text-gray-400 mb-8">
            <Link to="/" className="hover:underline">ホーム</Link> <ChevronRight size={10} />
            <Link to="/column" className="hover:underline">{post.category}</Link>
          </div>
          ...
          <article className="bg-white p-6 md:p-12 rounded-xl shadow-sm border border-gray-100">
            <header className="mb-10">
              ...
              <div className="flex items-center gap-4 text-[11px] text-gray-400 border-t border-gray-50 pt-4">
                ...
                <span className="flex items-center gap-1"><User size={12} /> SalonConciergeAdmin</span>
              </div>
            </header>
            ...
            <div className="prose prose-sm md:prose-base max-w-none text-gray-700 leading-relaxed space-y-8">
              ...
              <div className="bg-gray-50 border border-gray-200 p-8 rounded-lg">
                <p className="text-center font-bold text-gray-800 mb-4 text-lg">目次 <span className="text-sm font-normal text-gray-400">[閉じる]</span></p>
                <ol className="space-y-3 text-sm text-salon-pink font-bold list-decimal pl-6">
                  <li><span className="hover:underline cursor-pointer">営業時間を延ばすことで売上アップが期待できる理由</span></li>
                  <li><span className="hover:underline cursor-pointer">営業時間を長くする際の課題は受付対応</span></li>
                  <li><span className="hover:underline cursor-pointer">受付代行を活用して、機会損失を防ごう</span></li>
                  <li><span className="hover:underline cursor-pointer">まとめ：営業時間拡大×受付代行で売上を最大化</span></li>
                </ol>
              </div>
              ...
              <div className="bg-salon-pink/5 border border-salon-pink/20 p-6 rounded-lg text-sm">
                <Link to="/contact" className="text-salon-pink font-bold flex items-center gap-2 hover:underline">
              ...
              <div className="mt-20 border-t border-gray-100 pt-10">
                <p className="text-center text-xs font-bold text-gray-400 mb-6 uppercase tracking-widest">Share This Article</p>
                <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
                  <button className="bg-[#000000] text-white py-3 rounded flex flex-col items-center justify-center text-[10px] font-bold"><Twitter size={14} className="mb-1" /> X</button>
                  <button className="bg-[#1877f2] text-white py-3 rounded flex flex-col items-center justify-center text-[10px] font-bold"><Facebook size={14} className="mb-1" /> Facebook</button>
                  <button className="bg-[#0084ff] text-white py-3 rounded flex flex-col items-center justify-center text-[10px] font-bold">はてブ</button>
                  <button className="bg-[#ee4056] text-white py-3 rounded flex flex-col items-center justify-center text-[10px] font-bold">Pocket</button>
                  <button className="bg-[#06c755] text-white py-3 rounded flex flex-col items-center justify-center text-[10px] font-bold"><MessageCircle size={14} className="mb-1" /> LINE</button>
                  <button className="bg-gray-700 text-white py-3 rounded flex flex-col items-center justify-center text-[10px] font-bold"><Copy size={14} className="mb-1" /> Copy</button>
                </div>
              </div>
            </div>
          </article>
          ...
          <section className="mt-16">
            <h2 className="text-xl font-bold text-gray-800 mb-8 border-b-2 border-gray-100 pb-4">関連記事</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {BLOG_POSTS.slice(0, 4).map(post => (
                <Link key={post.id} to={`/column/${post.id}`} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex gap-4 hover:shadow-md transition-shadow">
                  <img src={post.image} className="w-24 h-24 object-cover rounded flex-shrink-0" alt={post.title} />
                  <div>
                    <h3 className="text-sm font-bold text-gray-800 leading-tight mb-2 line-clamp-2">{post.title}</h3>
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
            ...
            <ul className="space-y-4">
              {BLOG_POSTS.slice(0, 5).map(p => (
                <li key={p.id} className="pb-4 border-b border-gray-50 last:border-0">
                  <Link to={`/column/${p.id}`} className={`text-sm leading-relaxed transition-colors ${p.id === id ? 'text-salon-pink font-bold' : 'text-gray-600 hover:text-salon-pink'}`}>
                    {p.title}
                  </Link>
                </li>
              ))}
